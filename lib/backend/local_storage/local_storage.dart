...
const String processKey = "processKey";
...
const String couponKey = "couponKey";
...
const String identifierKey = "identifierKey";

const String trxTypeKey = "trxTypeKey";

const String smallLanguage = "smallLanguage";

const String saveEmailVerification = "saveEmailVerification";
...
class LocalStorage {
  ...
  static Future<void> saveTransactionType({required String type}) async {
    final box = GetStorage();
    await box.write(trxTypeKey, type);
  }

  static String getTransactionType() {
    return GetStorage().read(trxTypeKey) ?? '';
  }

  static Future<void> saveCoupon({required String type}) async {
    final box = GetStorage();
    await box.write(couponKey, type);
  }

  static String getCouponId() {
    return GetStorage().read(couponKey) ?? '';
  }
  ...
  static bool isRtl() {
    return GetStorage().read('RTL') ?? false;
  }

  static Future<void> saveEmailVerify({required bool value}) async {
    final box = GetStorage();
    await box.write(saveEmailVerification, value);
  }

  static bool isEmailVerify() {
    return GetStorage().read(saveEmailVerification) ?? false;
  }
  ...
  static Future<void> saveProcess({required bool value}) async {
    final box = GetStorage();

    await box.write(processKey, value);
  }

  static bool getProcess() {
    return GetStorage().read(processKey) ?? false;
  }
  ...
  static Future<void> saveIdentifierKey({required String key}) async {
    final box = GetStorage();
    await box.write(identifierKey, key);

    debugPrint('Identifier:$key');
  }
  ...
  static Future<void> saveColor({required HexColor color}) async {
    final box = GetStorage();
    await box.write(colorKey, color);
  }
  ...
  static String getIdentifierKey() {
    return GetStorage().read(identifierKey);
  }
  ...
  static HexColor getColor() {
    return GetStorage().read(colorKey) ?? HexColor('ffffffff');
  }
  ...
  static Future<void> removeCouponValue() async {
    final box = GetStorage();
    await box.remove(couponKey);
  }
}
